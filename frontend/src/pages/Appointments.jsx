...
const Appointments = () => {
  ...
  const docInf=doctors.find((item)=>item._id===docId);
  setDocInfo(docInf);
  // console.log(docInf);
  }
  ...
  for(let i=0;i<7;i++){
    ...
    //setting the date and time with the index
  ...
  return docInfo&&(
    <div>
      ...
      {/*-------- Doctors Available/Booking Slots ---------------*/}
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        <p>Booking Slots</p>
        <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
          {
            docSlots.length && docSlots.map((item,index)=>(
            <div onClick={() => setSlotIndex(index)} key={index} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex===index?'bg-[#5f6FFF] text-white':'border border-gray-300'}`}>
        ...
        <button onClick={bookAppointment} className='bg-[#5f6FFF] text-white test-sm font-light px-14 py-3 rounded-full my-6'>
          Book an appointment
        </button>
      </div>
      {/*-------- Related Doctors ---------------*/}
